...
Term.prototype.delete = function(options) {
  ...
  if (
    ![termTypes.GET, termTypes.GET_ALL, termTypes.FILTER].includes(
      self._query[0]
    ) &&
    !(options && options.forceTruncate)
  ) {
    throw new Error.ReqlDriverError(
      'Running delete is only allowed after get, getAll or filter. For truncating a table, pass { forceTruncate: true }.'
    );
  }
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if ((key !== 'returnChanges') && (key !== 'durability') && (key !== 'forceTruncate')) {
        throw new Error.ReqlDriverError('Unrecognized option `'+key+'` in `delete`', self._query, 'Available options are returnChanges <bool>, durability <string>, forceTruncate <bool>');
      }
    });
    term._query.push(new Term(self._r).expr(translateOptions({ ...options, forceTruncate: undefined }))._query);
  }
  return term;
}
